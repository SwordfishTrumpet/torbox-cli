...
class TorboxCli < Formula
  ...
  homepage "https://github.com/SwordfishTrumpet/torbox-cli"
  url "{URL}"
  sha256 "{SHA256}"
  license "MIT"
...
def main() -> None:
    ...
    parser.add_argument(
        "--sha256", required=True, help="SHA256 of the GitHub release sdist"
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    url = (
        f"https://github.com/SwordfishTrumpet/torbox-cli/"
        f"releases/download/v{args.version}/torbox-cli-{args.version}.tar.gz"
    )
